...
class ContainsSubnetValidator extends ConstraintValidator {	
    ...
    */
   	...
   	public function validate ($intIp , Constraint $constraint)
	{
	 ...
	 //$result=$repositullorio->getRepository('')->findAll();
			...
			if(!strcmp($ipValidate[0],"000")||!strcmp($ipValidate[0],"00")||!strcmp($ipValidate[0],"0")){
				$this->context->addViolation($constraint->message, array('%mac_address%'=> $intIp)); 
			}else if((!strcmp($ipValidate[0],"000")||!strcmp($ipValidate[0],"00")||!strcmp($ipValidate[0],"0"))&&($ipValidate[3]>254 || $ipValidate[3]<0)){
                $this->context->addViolation($constraint->message, array('%mac_address%'=> $intIp));
			}else if($ipValidate[3]>254 || $ipValidate[3]<0){
				$this->context->addViolation($constraint->message, array('%mac_address%'=> $intIp));
			}
			/*else if(!$result){
				$this->context->addViolation($constraint->message, array('%mac_address%'=> $intIp));
